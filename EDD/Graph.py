...
class Graph:
            ...
            for neighbor in range(self.V):
                if self.adj[current][neighbor] != float("inf") and not visited[neighbor]:
                    if has_visa or not self.visas[self.codes[neighbor]]:
                        auxList.append((path + [self.codes[neighbor]], stops + 1, cost + self.adj[current][neighbor]))
    ...
    def plot_graph(self):
        ...
        node_colors = ['#ff9999' if self.visas[self.codes[i]] else 'lightblue' for i in range(self.V)]

        nx.draw(G, pos, with_labels=True, labels={node: node for node in G.nodes()},
                node_color=node_colors, node_size=500, font_size=10, font_weight='bold')
    ...
    def plot_selected_path(self, path):
        ...
        edge_labels = nx.get_edge_attributes(G, 'weight')
        nx.draw(G, pos, with_labels=True, labels={node: node for node in G.nodes()},
                node_color='lightgray', node_size=500, font_size=10, font_weight='bold')
        ...
        dest_label = "Destino (Requiere Visa)" if self.visas[path[-1]] else "Destino"

        legend_elements = [Line2D([0], [0], marker='o', color='w', label='Requiere Visa',
                                  markerfacecolor='#ff9999', markersize=10),
                           Line2D([0], [0], marker='o', color='w', label='No Requiere Visa',
                                  markerfacecolor='lightblue', markersize=10),
                           Line2D([0], [0], marker='o', color='w', label='Ruta Recomendada',
